...
class HorizontalMRUV(BaseModel):
    def solve(self, s0, v0, a):
        ...
        stop_time = 0.0
        will_stop = False
        # Se velocidade e aceleração têm sinais opostos, ele vai parar e inverter
        if (v0 > 0 and a < 0) or (v0 < 0 and a > 0):
            will_stop = True
            stop_time = -v0 / a
            stop_pos = s0 + v0 * stop_time + 0.5 * a * stop_time**2
            status = "Movimento Retardado (Freando) até parar, depois Acelerado (Invertido)."
        elif a == 0:
             status = "Movimento Retilíneo Uniforme (MRU - Aceleração nula)."
             stop_pos = None
        else:
        ...
        steps = [
            {
                ...
                "text": f"O movimento é definido pela posição inicial S0={s0}m, velocidade inicial v0={v0}m/s e aceleração constante a={a}m/s². {status}",
                "equation_latex": f"S(t) = S_0 + v_0 t + \\frac{{at^2}}{{2}} \\Rightarrow {eq_pos_latex}"
            },
             {
                "step": 2, "title": "Função da Velocidade",
                "text": "A velocidade varia linearmente com o tempo devido à aceleração constante.",
                "equation_latex": f"v(t) = v_0 + at \\Rightarrow {eq_vel_latex}"
            }
        ]
        ...
        if will_stop:
             steps.append({
                ...
                "equation_latex": f"v(t) = 0 \\Rightarrow {v0} + {a}t = 0 \\Rightarrow t = {stop_time:.2f}\\text{{ s}}. \\quad S({stop_time:.2f}) = {stop_pos:.2f}\\text{{ m}}"
            })

        # 4. Gerando Arrays para Animação
        # Definimos um tempo de simulação fixo razoável (ex: 10s) ou um pouco após a parada
        ...
        position_s_array = []
        velocity_v_array = []
        acceleration_a_array = [] # Constante, mas útil para o frontend
